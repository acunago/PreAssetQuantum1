...
public class Emerald_AI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (OptimizedStateRef == OptimizedState.Active)
        {
                ...
                CurrentFollowTarget == null && CurrentHealth > 0 && BehaviorRef == CurrentBehavior.Pet)
            {
                ...
                else if (AIAgent.remainingDistance > AIAgent.stoppingDistance && !AIAgent.isStopped && !IsTurning)
                { //USed to be just else
    ...
    IEnumerator Attack()
    {
        ...
        if (CurrentTarget != null)
        {
            ...
            else if (IsRunAttack && UseRunAttacksRef == UseRunAttacks.Yes)
            { //Edited
                if (RunAttack != null)
                {
    ...
    {
        Debug.Log("CurrentTarget" + CurrentTarget.transform.name);
